 ...
 * 5. BN_dim * (BM_dim / TM_dim) == NUM_THREADS
 */
...
__global__ void sgemm_v2_kernel(
    ...
    static_assert(BN_dim * (BM_dim / TM_dim) == NUM_THREADS, 
    ...
    const unsigned int thread_row = thread_id / BN_dim * TM_dim;                        // One thread is responsible for calculating TM element in the block
    ...
    float acc_register[TM_dim + 1] = {0.};                                                 // Register to accumulate TM element for each thread
    ...
    for (unsigned int block_k = 0; block_k < num_block_tiles_k; ++block_k) {
        ...
        for (unsigned int k_inner = 0; k_inner < BK_dim; ++k_inner) {
            acc_register[TM_dim] = B_block_smem[k_inner * BN_dim + thread_col];            // An extra Register to avoid repeatedly copyin B_block_smem from shared memory
            for(unsigned int tm = 0; tm < TM_dim; ++tm){
                acc_register[tm] += A_block_smem[(thread_row + tm) * BK_dim + k_inner] * acc_register[TM_dim];
            }
    ...
    for (unsigned int tm = 0; tm < TM_dim; ++tm) {
        ...
        const unsigned int c_index = row_idx * C_stride + thread_col;
        C_tile[c_index] = alpha * acc_register[tm] + beta * C_tile[c_index];
    }
}
...
void benchmark_gemm(unsigned int M, unsigned int N, unsigned int K) {
    ...
    constexpr unsigned int BN_dim = 32;
    constexpr unsigned int BK_dim = 8;
    constexpr unsigned int TM_dim = 4;
    constexpr unsigned int NUM_THREADS = BM_dim * BN_dim / TM_dim;
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        dim3 block(BN_dim, NUM_THREADS / BN_dim);
        size_t shmem_size = (BM_dim * BK_dim + BK_dim * BN_dim) * sizeof(float);
        
        sgemm_v2_kernel<BM_dim, BN_dim, BK_dim, TM_dim, NUM_THREADS>
