...
__global__ void sgemm_v2_kernel(
    ...
    extern __shared__ __align__(32) float shmem[];
    ...
    for (unsigned int block_k = 0; block_k < num_block_tiles_k; ++block_k) {
        // Calculate global memory positions
        float* A_block_gmem = &A[block_m * BM_dim * A_stride + block_k * BK_dim];
        float* B_block_gmem = &B[block_k * BK_dim * B_stride + block_n * BN_dim];

        // Vectorized tile loading with implicit A transposition
        tileMemcpyVectorized<BM_dim, BK_dim, NUM_THREADS>(
            A_block_gmem, A_block_smem, A_stride);
        
        // Vectorized tile loading (no transposition)
        tileMemcpyVectorized<BK_dim, BN_dim, NUM_THREADS>(
            B_block_gmem, B_block_smem, B_stride);
        
        __syncthreads();

        // Register tile computation
        #pragma unroll
        for (unsigned int k_inner = 0; k_inner < BK_dim; ++k_inner) {
            ...
            load_matrix_tiles_to_registers<TM_dim, TN_dim, BK_dim, BN_dim>(
                A_block_smem, B_block_smem,
                A_register, B_register,
                thread_row, thread_col, k_inner);
...
void benchmark_gemm(unsigned int M, unsigned int N, unsigned int K) {
    ...
    constexpr unsigned int BM_dim = 128*2;
    constexpr unsigned int BN_dim = 128;
    constexpr unsigned int BK_dim = 32;
    constexpr unsigned int TM_dim = 16;
