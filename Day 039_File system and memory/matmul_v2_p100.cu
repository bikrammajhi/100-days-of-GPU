...
__global__ void sgemm_v2_kernel(
    ...
    for (unsigned int block_k = 0; block_k < num_block_tiles_k; ++block_k) {
        ...
        tileMemcpy(A_block_gmem, A_block_smem, A_stride, BM_dim, BK_dim);
        tileMemcpy(B_block_gmem, B_block_smem, B_stride, BK_dim, BN_dim);
        __syncthreads();

        #pragma unroll
        for (unsigned int k_inner = 0; k_inner < BK_dim; ++k_inner) {
            acc_register += A_block_smem[thread_row * BK_dim + k_inner] 
                         * B_block_smem[k_inner * BN_dim + thread_col];
        }
        __syncthreads();
    }
...
void benchmark_gemm(unsigned int M, unsigned int N, unsigned int K) {
    ...
    constexpr unsigned int NUM_THREADS = BM_dim * BN_dim;
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        sgemm_v2_kernel<BM_dim, BN_dim, BK_dim, NUM_THREADS>
